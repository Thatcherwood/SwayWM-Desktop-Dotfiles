...
def parse_wpctl_status():
    ...
    # strip the * from the default source and instead append "- Default" to the end. Looks neater in the tofi list this way.
    for index, source in enumerate(sources):
...
# get the list of sources ready to put into tofi - highlight the current default source
output = ''
...
# Call tofi and show the list. take the selected source name and set it as the default source
tofi_command = f"echo '{output}' | tofi"
tofi_process = subprocess.run(tofi_command, shell=True, encoding='utf-8', stdout=subprocess.PIPE, stderr=subprocess.PIPE)

if tofi_process.returncode != 0:
    print("User cancelled the operation.")
    exit(0)

selected_source_name = tofi_process.stdout.strip()
sources = parse_wpctl_status()
