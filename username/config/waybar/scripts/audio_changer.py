...
def parse_wpctl_status():
    ...
    # strip the * from the default sink and instead append "- Default" to the end. Looks neater in the tofi list this way.
    for index, sink in enumerate(sinks):
        if sink.startswith("*"):
...
# get the list of sinks ready to put into tofi - highlight the current default sink
output = ''
...
# Call tofi and show the list. take the selected sink name and set it as the default sink
...
selected_sink_name = tofi_process.stdout.strip()
sinks = parse_wpctl_status()
